...
class Tournament(var attributes: TournamentAttributes) {
    ...
    private fun Player.normalize() {
        ...
        if (player.tournamentPouch) {
            player.runePouch.set(0, null)
            player.runePouch.set(1, null)
            player.runePouch.set(2, null)
            player.tournamentPouch = false
        }
        if (player.cachedRunePouchTypes != 0) {
            player.runePouch.empty(false)
            ...
            if (firstRuneType != 0) {
                ...
                player.runePouch.set(0, Item(rune.id, Config.RUNE_POUCH_LEFT_AMOUNT.get(player)))
            }

            val secondRuneType = Config.RUNE_POUCH_MIDDLE_TYPE.get(player)
            if (secondRuneType != 0) {
                val rune = Rune.VALUES[secondRuneType - 1]
                player.runePouch.set(1, Item(rune.id, Config.RUNE_POUCH_MIDDLE_AMOUNT.get(player)))
            }

            val thirdRuneType = Config.RUNE_POUCH_RIGHT_TYPE.get(player)
            if (thirdRuneType != 0) {
                val rune = Rune.VALUES[thirdRuneType - 1]
                player.runePouch.set(2, Item(rune.id, Config.RUNE_POUCH_RIGHT_AMOUNT.get(player)))
            }
    ...
    private fun Player.resolvePrize() {
        when (contestants) {
            ...
            2 -> {
                ...
                message(Color.RED.wrap("Congratulations! You have placed second within the tournament. You have won 2.5M GP, and a mystery box!"))
            }
            1 -> {
                inventory.add(COINS_995, 5_000_000)
                inventory.add(290, 1)
                message(Color.RED.wrap("Congratulations! You have placed first within the tournament. You have won 5M GP and a super mystery box!"))
                message(Color.RED.wrap("Your winnings have been added to your bank."))
            }
            else -> {
