...
object TournamentManager {
    ...
    init {
         ...
         */
        whenObjClick(24085, 1) { player, _ ->
            if (player.position.z == 3 && Lobby.players.contains(player)) {
                player.event {
                    if (player.options("Leave Tournament?", "No") == 1) {
                        player.leave()
                    }
                }
            }
    ...
    fun Player.join() {
        event {
            ...
            activeTournament?.participants?.forEach { participant ->
                if (ipInt == participant.ipInt) {
