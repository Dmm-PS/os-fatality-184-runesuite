...
class LastManStandingSession(private val queue: LastManStandingQueue) {
    ...
    private fun Player.normalize() {
        ...
        if (tournamentPouch) {
            runePouch.set(0, null)
            runePouch.set(1, null)
            runePouch.set(2, null)
...
//            runePouch.empty(false)
            ...
            if (firstRuneType != 0) {
                ...
                runePouch.set(0, Item(rune.id, Config.RUNE_POUCH_LEFT_AMOUNT.get(this)))
            }

            val secondRuneType = Config.RUNE_POUCH_MIDDLE_TYPE.get(this)
            if (secondRuneType != 0) {
                val rune = Rune.VALUES[secondRuneType - 1]
                runePouch.set(1, Item(rune.id, Config.RUNE_POUCH_MIDDLE_AMOUNT.get(this)))
            }

            val thirdRuneType = Config.RUNE_POUCH_RIGHT_TYPE.get(this)
            if (thirdRuneType != 0) {
                val rune = Rune.VALUES[thirdRuneType - 1]
                runePouch.set(2, Item(rune.id, Config.RUNE_POUCH_RIGHT_AMOUNT.get(this)))
            }
    ...
    private fun Player.setGear() {
        ...
        runePouch.set(0, Item(Rune.WATER.id, 16000))
        runePouch.set(1, Item(Rune.DEATH.id, 16000))
        runePouch.set(2, Item(Rune.BLOOD.id, 16000))
