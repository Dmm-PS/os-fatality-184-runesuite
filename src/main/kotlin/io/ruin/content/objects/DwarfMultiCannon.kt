...
object DwarfMultiCannon {
    ...
    fun Player.setup() {
        ...
        event {
            ...
            val pos = position.translate(0, 1)
            animate(827)
    ...
    private fun Player.preRequirements(): Boolean {
        ...
        if (combat.isDefending(6) || combat.isAttacking(6)) {
            message("You cannot do this while in combat!")
            return false
        }
        if (!Tile.allowObjectPlacement(player.position) || player.position.inBounds(EDGEVILLE)) {
