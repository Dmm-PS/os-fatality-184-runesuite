...
public class Player extends PlayerAttributes {
    ...
    public boolean isSeniorModerator() {
        return isGroup(PlayerGroup.SENIOR_MODERATOR) || isGroup(PlayerGroup.DISCORD_MANAGER) || isGroup(PlayerGroup.EVENTS_MODERATOR) || isGroup(PlayerGroup.ADMINISTRATOR);
    }

    public boolean isDiceHost() {
        return player.diceHost || player.isGroup(PlayerGroup.DICE_HOST);
    }

    public boolean isPKMode() { return player.isGroup(PlayerGroup.PK_MODE); }

    public boolean isAdmin() {
        return isGroup(PlayerGroup.OWNER) || isGroup(PlayerGroup.DEVELOPER) || isGroup(PlayerGroup.EXECUTIVE_MANAGER)
                || isGroup(PlayerGroup.CHIEF_DEVELOPMENT_OFFICER) || isGroup(PlayerGroup.COMMUNITY_MANAGER)
                || isGroup(PlayerGroup.ADMINISTRATOR);
    }

    public boolean isStaff() {
        return  isGroup(PlayerGroup.MODERATOR) ||  isGroup(PlayerGroup.SUPPORT) || isGroup(PlayerGroup.SENIOR_MODERATOR)
                || isGroup(PlayerGroup.DISCORD_MANAGER) || isGroup(PlayerGroup.EVENTS_MODERATOR) || isGroup(PlayerGroup.ADMINISTRATOR)
                || isGroup(PlayerGroup.OWNER) || isGroup(PlayerGroup.DEVELOPER) || isGroup(PlayerGroup.EXECUTIVE_MANAGER)
                || isGroup(PlayerGroup.CHIEF_DEVELOPMENT_OFFICER) || isGroup(PlayerGroup.COMMUNITY_MANAGER)
                || isGroup(PlayerGroup.ADMINISTRATOR);
    }
    ...
    public void start() {
         ...
         * Messages
         */
        sendMessage("Welcome to " + World.type.getWorldName() + "." + (" On this server, you die."));
