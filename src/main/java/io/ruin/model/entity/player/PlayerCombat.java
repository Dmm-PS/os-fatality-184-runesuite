...
import io.ruin.model.item.actions.impl.chargable.DragonfireShield;
import io.ruin.model.item.actions.impl.chargable.TridentOfTheSeas;
...
public class PlayerCombat extends Combat {
    ...
    public boolean canAttack(Entity target, boolean message) {
        ...
        /*
         * Check to ensure PK Mode players can't attack outside wilderness.
         */
        if (target.isNpc() && target.npc.getId() != 2668) {
            if (player.isPKMode()
                    && player.wildernessLevel < 1
                    && Objects.isNull(player.pestGame)
                    && Objects.isNull(player.fightCaves)
                    && !WarriorsGuild.WARRIORS_GUILD.intersects(target.npc.getBounds())
                    && !WarriorsGuild.WARRIORS_GUILD_BASEMENT.intersects(target.npc.getBounds())) {
                player.sendMessage("You can't attack that npc.");
                return false;
            }
        }
    ...
    private void attackWithMagicWeapon() { // trash
        ItemDef weapon = player.getEquipment().getDef(Equipment.SLOT_WEAPON);
        boolean swamp = weapon.id == 12899;
        if (weapon.id == TridentOfTheSeas.UNCHARGED || weapon.id == 12900) {
            player.sendMessage("Your trident has no charges left!");
            updateLastAttack(4);
            return;
        }
        if (swamp || weapon.id == TridentOfTheSeas.CHARGED || weapon.id == TridentOfTheSeas.FULLY_CHARGED) {
            if (target.player != null) {
            ...
            Hit hit = new Hit(player, AttackStyle.MAGIC, getAttackType()).randDamage(getTridentMaxDamage(swamp)).clientDelay(duration).setAttackWeapon(weapon);
    ...
    public void startDeath(Hit killHit) {
        ...
        player.startEvent(event -> {
            ...
            if(player.deathEndListener != null) {
                ...
                player.getMovement().teleport(3075, 3503, 0);
