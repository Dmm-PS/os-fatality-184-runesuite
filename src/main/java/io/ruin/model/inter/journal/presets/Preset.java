...
/*        if (player.pvpInstancePosition != null && !player.pvpAttackZone) {
        ...
        if(!player.isPKMode() && this.free) {
            player.dialogue(new MessageDialogue("You need to be playing the PK gamemode to use this preset."));
            return false;
        }
    ...
    private void withdrawItems(Player player, boolean inventory) {
        ...
        for (int i = 0; i < items.length; i++) {
            ...
            if (presetItem.getAmount() >= bankItem.getAmount()) {
                ...
                container.set(i, new Item(presetItem.getId(), presetItem.getAmount(), bankItem.getUniqueValue()));
