...
public class TridentOfTheSeas {
    ...
    public static void check(Player player, Item item) {
        if (item.getId() == FULLY_CHARGED)
            ...
            player.sendMessage("Your Trident of the Seas has " + item.getUniqueValue() + " charges remaining.");
    }
    ...
    public static void charge(Player p, Item trident, Item other) {
        int currentCharges = trident.getId() == 11908 ? 0 : trident.getUniqueValue();
        ...
        if (chargesToAdd + currentCharges == 2500) {
            ...
            trident.setUniqueValue(chargesToAdd + currentCharges);
