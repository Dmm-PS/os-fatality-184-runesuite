...
import io.ruin.model.entity.npc.actions.edgeville.SigmundTheMerchant;
...
public class Trade extends ItemContainer {
    ...
    private void updateSigmund() {
        ...
        for(Item item : getItems()) {
            if(item != null)
                price += (long) SigmundTheMerchant.getPrice(player, item) * item.getAmount();
        }
    ...
    public void offer(Item item, int amount) {
        ...
        if(targetTrade.sigmund) {
            ...
            sendMessage("You can't trade this item.");
            if(player.isAdmin())
                player.sendMessage("  uniqueValue=" + item.getUniqueValue());
            return;
        }
