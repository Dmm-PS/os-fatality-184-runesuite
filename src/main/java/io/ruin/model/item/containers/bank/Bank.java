...
public class Bank extends ItemContainerG<BankItem> {
    ...
    public int add(int id, int amount, int uniqueValue) {
        ...
        for(int slot = 0; slot < items.length; slot++) {
            ...
            if(item.getId() == id && uniqueValue == 0 && item.getUniqueValue() == 0) {
        ...
        if(blankItem != null) {
            ...
            blankItem.setUniqueValue(uniqueValue);
            return amount;
        }
        if(freeSlot != -1) {
            set(freeSlot, new BankItem(id, amount, uniqueValue, tab));
            if(tab != 0) sort();
            return amount;
        }
        return 0;
    ...
    @Override
    protected BankItem newItem(int id, int amount, int uniqueValue) {
        return new BankItem(id, amount, uniqueValue, 0);
    }
    ...
    private void fillBank() {
        sortAfter(() -> {
            ...
            for(int slot = items.length - 1; slot >= 0; slot--) {
                ...
                if(item == null) {
                    filled++;
                    item = new BankItem(FILLER_ID, 1, 0, tab);
                    item.sortSlot = 65535;
                    set(slot, item);
                    sort();
    ...
    static {
        ...
        InterfaceHandler.register(Interface.BANK, h -> {
            ...
            h.actions[34] = (SimpleAction) p -> Config.BANK_DEFAULT_QUANTITY.set(p, 3);
