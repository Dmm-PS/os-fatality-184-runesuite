...
public class StatList {
    ...
    public void addXp(StatType type, double amount, boolean useMultiplier) {
        ...
        if (player.isPKMode() && (statId > 6 && type != StatType.Agility)) {
            player.sendMessage("PK Mode accounts cannot gain skilling experience.");
            return;
        }
        if(useMultiplier) {
        ...
        if(type.defaultXpMultiplier == -1) { //combat stat
            if(player.combatXpRate == 0)
                return;
            if(useMultiplier) {
                if(stat.fixedLevel >= 99)
                    amount *= Math.min(player.combatXpRate, type.after99XpMultiplier);
                else
                    amount *= player.combatXpRate;
            }
        } else { //non-combat stat
            if(useMultiplier) {
                if(stat.fixedLevel >= 99)
                    amount *= type.after99XpMultiplier;
                else
                    amount *= type.defaultXpMultiplier;
            }
        }
        /**
    ...
    public void process() {
        ...
        boolean rapidHeal = player.getPrayer().isActive(Prayer.RAPID_HEAL) || CapePerks.wearsHPCape(player);
