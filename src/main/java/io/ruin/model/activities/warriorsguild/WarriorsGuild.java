    ...
    static {
        ...
        for(Doors door : Doors.values()) {
            ObjectAction.register(door.id, door.doorX, door.doorY, door.doorZ, "open", (player, obj) -> player.startEvent(event -> {
                ...
                if((player.getAbsX() < obj.x && door == Doors.CYCLOPS_NORTH || door == Doors.CYCLOPS_SOUTH) || (player.getAbsX() == obj.x && door == Doors.CYCLOPS_BASEMENT)) {
                    Item tokens = player.getInventory().findItem(8851);
                    if(!CapePerks.wearsAttackCape(player) && (tokens == null || tokens.getAmount() < 100)) {
