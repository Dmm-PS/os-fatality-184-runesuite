        ...
        if (isTask(player, npc)) {
            ...
            if (Wilderness.getLevel(npc.getPosition()) > 0) {
                npc.deathEndListener = (DeathListener.SimpleKiller) killer -> {
                    /*if (Random.rollDie((int) Math.ceil(getKeyDropRate(npc) * 1000), 1)) {
                        new GroundItem(23490, 1).owner(killer.player).position(npc.getPosition()).spawn();*/ //TODO: Disabled until we get the new client...
