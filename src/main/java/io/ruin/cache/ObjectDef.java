...
public class ObjectDef {
    ...
    public static void load() {
        ...
        for (int id = 40000; id < 40000 + LOADED_EXTRA.length; id++) {
            ObjectDef def = new ObjectDef();
            def.id = id;
            def.decode(new InBuffer(EMPTY_BUFFER));
            if (def.someFlag) {
                def.clipType = 0;
                def.tall = false;
            }
            ...
            LOADED_EXTRA[id - 40000] = def;
        }
    }

    public static void forEach(Consumer<ObjectDef> consumer) {
        for (ObjectDef def : LOADED) {
            if (def != null)
                consumer.accept(def);
        }
        for (ObjectDef def : LOADED_EXTRA) {
            if (def != null)
                consumer.accept(def);
        }
    }
    ...
    public static ObjectDef get(int id) {
        if (id >= 40000 && id - 40000 < LOADED_EXTRA.length)
            return LOADED_EXTRA[id - 40000];
