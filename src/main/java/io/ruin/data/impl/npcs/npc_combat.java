...
public class npc_combat implements DataFile {
    ...
    */
            ...
            list.forEach(info -> {
                for (int id : info.ids) {
                    ...
                    if (def.combatInfo != null) {
                        ServerWrapper.logWarning(def.id + ": " + def.name + " already has combat info set!");
                    }
                    def.combatInfo = info;
    ...
    private static void toJson(File dumpFile) throws IOException {
        ...
        npcInfo.forEach(lines -> {
            ...
            /**
             * Okay.. all web data parsing is done.. THANK GOD
