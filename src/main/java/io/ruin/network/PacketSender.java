...
public class PacketSender {
    ...
    public void sendItem(int parentId, int childId, int itemId, int amount) {
        OutBuffer out = new OutBuffer(11).sendFixedPacket(75)
                .addInt(parentId << 16 | childId)
                .addLEInt(amount)
                .addShort(itemId);
        write(out);
    ...
    public void sendItems(int interfaceHash, int containerId, Item[] items, int length) {
        OutBuffer out = new OutBuffer(10 + (length * 10)).sendVarShortPacket(49)
        ...
        for(int slot = 0; slot < length; slot++) {
            ...
            if(item == null || item.getId() < 0) {
                ...
                out.addByteA(Math.min(255, item.getAmount()));
                if (item.getAmount() >= 255) {
                    out.addLEInt(item.getAmount());
                }
            }
        }
        write(out);
    ...
    public void updateItems(int interfaceHash, int containerId, Item[] items, boolean[] updatedSlots, int updatedCount) {
        OutBuffer out = new OutBuffer(10 + (updatedCount * 10)).sendVarShortPacket(1)
                .addInt(interfaceHash)
                .addShort(containerId);
    ...
    public void resetMapFlag() {
        OutBuffer out = new OutBuffer(2).sendFixedPacket(25)
                .addByte(-1)
                .addByte(-1);
        write(out);
    }
    ...
    public void sendShop(String title, ShopItem... items) {
        OutBuffer out = new OutBuffer(3 + Protocol.strLen(title) + 2 + (items.length * 6)).sendVarShortPacket(88)
                .addString(title)
