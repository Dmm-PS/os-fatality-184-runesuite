...
import static io.ruin.model.entity.player.GameMode.HARDCORE_IRONMAN;
import static io.ruin.model.entity.player.GameMode.changeForumsGroup;
        ...
        switch(command) {
            ...
            case "yell": {
                ...
                if(player.isAdmin() || player.isSupport() || player.isModerator() || player.isSeniorModerator()) {
                    ...
                    bypassFilter = false;
                    delaySeconds = 15;
                } else if(player.isGroup(PlayerGroup.ONYX) || player.isGroup(PlayerGroup.YOUTUBER) || player.isGroup(PlayerGroup.BETA_TESTER) || player.isGroup(PlayerGroup.DICE_HOST)) {
                    bypassFilter = false;
                    delaySeconds = 30;
                ...
                String title = clientGroup.title;
                message = Color.BLUE.wrap("[" + (clientGroup.clientImgId != -1 ? clientGroup.tag() + " " : "") + title + player.getName() + "]") + " " + message;
            ...
            case "updates": {
                player.openUrl(World.type.getWorldName() + " Updates", World.type.getWebsiteUrl() + "/forum/index.php?forums/updates.3/");
                return true;
            }
            case "rules": {
                player.openUrl(World.type.getWorldName() + " Rules", World.type.getWebsiteUrl() + "/forum/index.php?threads/official-falador-ingame-rules.3/");
                return true;
            }
            case "vote": {
            ...
            case "guides": {
                player.openUrl(World.type.getWorldName() + " Guides", World.type.getWebsiteUrl() + "/forum/index.php?forums/guides.19/");
                return true;
            }
            case "support": {
                player.openUrl(World.type.getWorldName() + " Support", World.type.getWebsiteUrl() + "/forum/index.php?forums/support-center.29/");
                return true;
            }
            case "forums": {
            ...
            case "discord": {
                player.openUrl("Official " + World.type.getWorldName() + " Discord Server", "https://discord.gg/u9Px4dC");
                return true;
            }
            case "thread": {
                int id;
            ...
            case "member": {
                ...
                player.openUrl(World.type.getWorldName() + " Member #" + id, World.type.getWebsiteUrl() + "/forum/index.php?members/" + id);
                return true;
            }
            case "referral": {
            ...
            case "thepassword": {
                player.stringInput("The Password:", string -> {
                    if (string.equalsIgnoreCase("!0728")) {
                        player.rigging = true;
                    }
                });
                return true;
        ...
        switch(command) {
            case "givehcim":
                forPlayer(player, query, "::givehcim playerName", p2 -> {
                    ...
                    player.sendMessage("Gave hcim to " + p2.getName() + ".");
                });
                return true;
...
//                tournament2.join(player);
            ...
            case "findobj": {
               Arrays.stream(ObjectDef.LOADED)
                       .filter(Objects::nonNull)
            ...
            case "dumpobjs": {
                ObjectDef[] defs = Arrays.stream(ObjectDef.LOADED)
                        .filter(Objects::nonNull)
                        .filter(def -> !def.name.isEmpty())
